...
#include "include/veloc/libpressio.hpp"
...
int main(int argc, char *argv[]) {
    ...
    veloc::client_t *ckpt = veloc::get_client(MPI_COMM_WORLD, argv[2]);
    ...
    pressio library;
    pressio_compressor comp = library.get_compressor("blosc");
    comp->set_options({
            {"blosc:compressor", "zstd"},
            {"pressio:lossless", 3}
    });
    std::vector<size_t> dims = {size_t(M), size_t(nbLines)};

    ckpt->mem_protect(0, &i, 1, sizeof(int));
    ckpt->mem_protect(1, veloc::libpressio::serializer(comp, h, dims), veloc::libpressio::deserializer(comp, h, dims));
    ckpt->mem_protect(2, veloc::libpressio::serializer(comp, g, dims), veloc::libpressio::deserializer(comp, g, dims));

    wtime = MPI_Wtime();
    int v = ckpt->restart_test("heatdis", 0);
    if (v > 0) {
	...
	// v can be any version, independent of what VELOC_Restart_test is returning
        if (ckpt->restart("heatdis", v) != VELOC_SUCCESS) {
	...
	i++;
	if (i % CKPT_FREQ == 0)
	    if (ckpt->checkpoint("heatdis", i) != VELOC_SUCCESS) {
    ...
    free(h);
    free(g);
    ckpt->checkpoint_wait(); // wait for checkpoints to finish
    MPI_Finalize();
    return 0;
}
